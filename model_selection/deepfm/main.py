...
def _run_base_model_dfm(dfTrain = None, dfTest = None, trainfile = None, testfile = None, dfm_params = None):
    ...
    if dfm_params["use_fm"] and dfm_params["use_deep"]:
        ...
        clf_str = "DNN"
    print("%s: %.5f (%.5f)"%(clf_str, err_results_cv.mean(), err_results_cv.std()))
    filename = "%s_Mean%.5f_Std%.5f.csv"%(clf_str, err_results_cv.mean(), err_results_cv.std())
...
def _make_submission(ids, y_pred, filename="submission.csv"):
    pd.DataFrame({"id": ids, "target": y_pred.flatten()}).to_csv(
        os.path.join(config.SUB_DIR, filename), index=False)
