...
class NFM(BaseEstimator, TransformerMixin):
    def __init__(self, feature_size, field_size,
                 embedding_size=128, deep_layers=[64],
                 dropout=[0.4, 0.4],
                 deep_layers_activation=tf.nn.relu,
                 epoch=10, batch_size=256,
                 learning_rate=0.001, optimizer_type="adam",
                 batch_norm=False, batch_norm_decay=0.995,
                 verbose=0, random_seed=2020,
                 init_weights = "normal_kaiming",
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.dropout = tf.placeholder(tf.float32, shape=[None], name="dropout")
            ...
            self.y_first_order = tf.nn.dropout(self.y_first_order, self.dropout[0]) # None * F

            # ---------- second order term and deep component---------------
            # sum_square part
            ...
            self.y_second_order = tf.nn.dropout(self.y_second_order, self.dropout[0])  # None * K
            ...
            for i in range(0, len(self.deep_layers)):
                ...
                self.y_deep = tf.nn.dropout(self.y_deep, self.dropout[1+i]) # dropout at each Deep layer
