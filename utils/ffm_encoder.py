...
def hashstr(s, nr_bins=1e+6):
    return int(hashlib.md5(s.encode('utf8')).hexdigest(), 16) % (int(nr_bins) - 1) + 1
...
class FfmEncoder():
    ...
    def gen_hashed_fm_feats(self, feats):
        ...
        for (field, feat) in feats:
            if '$' in feat:
                for item in feat.split('$'):
                    feats.append('{0}:{1}:1'.format(field, hashstr(item, 1e+6)))
            else:
                if '.' in feat:
                    feats.append('{0}:1:{1}'.format(field, feat))
                else:
                    feats.append('{0}:{1}:1'.format(field, hashstr(feat, 1e+6)))
        return feats
